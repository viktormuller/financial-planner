...
export class PensionStrategy {
  apply(
    ...
    var ret: MonetaryValue = new MonetaryValue(grossIncome.value);
  ...
  withdraw(hh: Household, amount: MonetaryValue, year: number): MonetaryValue {
    ...
    while (unfundedBalance.value > 0) {
      ...
      var adultsWithBalance: number = hh.adults.filter(
        adult =>
          adult.pensionAccount.closingValue(year).value > 0 &&
          adult.job.endYear < year &&
          adult.yearOfJoining >= year
      ).length;
